...
import { testConduit } from "./user-authentication-fixture";
testConduit.describe("User Authentication", () => {
  testConduit(
    "101 signUp user - We should be able to sign up a new user",
    async ({ page, goToTheMainPage }) => {
      await goToTheMainPage();
  ...
  testConduit(
    "Sign in user - should sign in with an existing user and create an article",
    async ({ page, goToTheMainPage }) => {
      await goToTheMainPage();
